...
if (!$g_user->is_logged_in()) {
	...
	$files = array();
	$dir = opendir("testbank/Nutritional Science");
	while (false !== ($filename = readdir($dir))) {
		if (is_file("testbank/Nutritional Science/$filename")) {
			$files[] = $filename;
		}
	}
	natcasesort($files);
	foreach ($files as $filename) {
		...
		echo "<li><a href=\"testbank/Nutritional Science/$escaped_filename\">$escaped_filename</a></li>\n";
	}
	echo "</ul>\n";
	echo <<<HEREDOC
