...
if (!$g_user->is_logged_in() || !$g_user->permit("admin change passphrase")) {
	...
	$id = $_REQUEST['cancel'];
	$query = new Query(sprintf("UPDATE apo_fellowship_buddy SET canceled=TRUE WHERE id=%d", $id));
	header( 'Location: http://members.calaphio.com/admin_fellowship_buddy_create.php' );
	...
	$query = new Query(sprintf("INSERT INTO apo_fellowship_buddy SET begin='%s', end='%s', user_id=%d, buddy_id = %d" , date( 'Y-m-d H:i:s', $start_time), date( 'Y-m-d H:i:s', $end_time), $_REQUEST['person1'], $_REQUEST['person2']));
}
...
$buddies = "";
$query = new Query(sprintf("SELECT canceled, B.id, B.user_id, B.buddy_id, begin, end, U1.firstname AS firstname, U1.lastname AS lastname, U2.firstname AS firstname2, U2.lastname AS lastname2 FROM apo_fellowship_buddy B, apo_users U1, apo_users U2 WHERE B.user_id = U1.user_id AND B.buddy_id = U2.user_id ORDER BY firstname ASC"));
while ($row = $query->fetch_row()) {
	if (!$row['canceled']) {
		$person1 = $row['user_id'];
		...
		$begin = $row['begin'];
		$end = $row['end'];
		$buddy_link = "apo_fellowship_buddy.php" . "?id=" . $id; 
		$remove_buddy_link = "admin_fellowship_buddy_create.php" . "?cancel=" . $id;
...
echo <<<DOCHERE_print_fellowship_buddy_create
	<h2>Create Fellowship Buddy Pair </h2>
	...
	<h2 style="margin-top:1em"> Current Fellowship Buddies </h2>
	<div style="margin-top:1em">
		$buddies
	</div>
DOCHERE_print_fellowship_buddy_create;
