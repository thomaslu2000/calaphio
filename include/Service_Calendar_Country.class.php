...
<select name="timestamp">$select_month_options</select> <input type="submit" value="Go!" /> <a href="service_calendar_country.php?year=$prev_year&month=$prev_month">Prev</a> <a href="service_calendar_country.php?year=$next_year&month=$next_month">Next</a>
...
<select name=navi onkeypress="location.href = this.form.navi.options[this.form.navi.selectedIndex].value">
	...
	<option value="#" selected="selected">Service Calendar - Country
...
DOCHERE_print_upcoming_events;
	...
	function query_range($begin, $end) {
		...
		return new Query(sprintf("SELECT %s FROM %scalendar_event%s WHERE %s AND type_service_country = 1 ORDER BY date ASC", $select_expression, TABLE_PREFIX, $join_expression, $where_expression));
	}
}