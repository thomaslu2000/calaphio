...
if (!$g_user->is_logged_in()){
	trigger_error("You must be logged in to view the events you are attending.", E_USER_ERROR);
}
...
<select name="timestamp">$select_month_options</select> <input type="submit" value="Go!" /> <a href="attending_calendar.php?year=$prev_year&month=$prev_month">Prev</a> <a href="attending_calendar.php?year=$next_year&month=$next_month">Next</a>
...
<select name=navi onkeypress="location.href = this.form.navi.options[this.form.navi.selectedIndex].value">
	...
	<option value="#" selected="selected">Attending Calendar
</select>
...
DOCHERE_print_upcoming_events;
	...
	function query_range($begin, $end) {
		...
		return new Query(sprintf("SELECT %s FROM %scalendar_event%s WHERE %s AND %d in (SELECT user_id from apo_calendar_attend where apo_calendar_event.event_id = apo_calendar_attend.event_id) ORDER BY date ASC", $select_expression, TABLE_PREFIX, $join_expression, $where_expression, $g_user->data['user_id']));
	}
}
}