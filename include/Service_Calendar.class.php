...
<select name=navi onkeypress="location.href = this.form.navi.options[this.form.navi.selectedIndex].value">
	...
	<option value="#" selected="selected">Service Calendar
...
DOCHERE_print_upcoming_events;
	...
	function query_range($begin, $end) {
		...
		return new Query(sprintf("SELECT %s FROM %scalendar_event%s WHERE %s AND (type_service_campus = 1 or type_service_chapter = 1 or type_service_community = 1 or type_service_country = 1 or type_fundraiser = 1) ORDER BY date ASC", $select_expression, TABLE_PREFIX, $join_expression, $where_expression));
	}
}