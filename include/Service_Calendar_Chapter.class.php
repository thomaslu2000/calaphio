 ...
 */
define('CALENDAR_POPUP_WIDTH', 550);
define('CALENDAR_POPUP_HEIGHT', 560);
...
class Calendar {
	...
	function print_add_evaluation() {
		...
		$odd_row = true;
		$attendee_count = 0;
		$query = new Query(sprintf("SELECT %scalendar_attend.user_id, firstname, lastname, attended, flaked, chair, driver, hours FROM %scalendar_attend JOIN %susers USING (user_id) WHERE event_id=%d ORDER BY signup_time",
...
<td axis="hours" class="center"><input type="checkbox" name="hours$user_id"$hours_selected /> <input type="text" name="hoursAmount$user_id" size="2" value="$hours" /></td>
</tr>
...
DOCHERE_print_add_evaluation_attendees;
		...
		while ($row = $query->fetch_row()) {
			...
			if ($row['type'] == 'numerical') {
				...
				$response = $row['text_response'] ? $row['text_response'] : "";
...
<div class="numerical"><input type="text" name="event_hours" size="2" /></div>
$questions
<div id="eval_submit">
...
<tr>
  <td class="fieldName" axis="Field Name" rowspan="6">Event Type</td>
...
DOCHERE_edit_event;
	...
	function print_evaluation() {
		...
		while ($row = $query->fetch_row()) {
			...
			$hours = $row['hours'] ? "<span style=\"font-weight: bold\">$row[hours]</span>" : "<span style=\"font-weight: bold; color: red;\">???</span>";
			$attendees .= <<<DOCHERE
<tr class="$row_class">
...
DOCHERE_print_evaluation;
	...
	function print_event() {
		...
		if ($row = $query->fetch_row()) {
			...
			$query = new Query(sprintf("SELECT %scalendar_attend.user_id, email, firstname, lastname, signup_time, driver, pledgeclass, phone, cellphone, chair FROM %scalendar_attend JOIN %susers USING (user_id) WHERE event_id=%d ORDER BY signup_time", TABLE_PREFIX, TABLE_PREFIX, TABLE_PREFIX, $event_id));
			$attendees = "";
			...
			while ($row = $query->fetch_row()) {
				...
				if ($signup_limit == 0 || $attendee_count <= $signup_limit) {
					$attendees .= sprintf("<tr><td axis=\"name\">%d. %s %s</td><td axis=\"pledgeclass\">%s</td><td axis=\"driving\">%s</td><td axis=\"phone\">%s</td><td axis=\"cell\">%s</td><td axis=\"signup\">%s</td>$remove_attendee</tr>\r\n",
						$attendee_count, $row['firstname'], $row['lastname'], $row['pledgeclass'], $driving, $row['phone'], $row['cellphone'], $signup_time);
				} else {
					$waitlist .= sprintf("<tr><td axis=\"name\">%d. %s %s</td><td axis=\"pledgeclass\">%s</td><td axis=\"driving\">%s</td><td axis=\"phone\">%s</td><td axis=\"cell\">%s</td><td axis=\"signup\">%s</td>$remove_attendee</tr>\r\n",
						$attendee_count, $row['firstname'], $row['lastname'], $row['pledgeclass'], $driving, $row['phone'], $row['cellphone'], $signup_time);
				}
			}
			...
			$max_chairs = ceil((is_numeric($signup_limit) && $signup_limit ? min($signup_limit, $attendee_count) : $attendee_count) / 10);
			...
			if ($row = $query->fetch_row()) {
				$signup = '';
				$make_me_chair = !$signup_hardlock && !$row['chair'] && $chair_count < $max_chairs ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Make Me Chair\">Make Me Chair</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : '';
				$take_me_off = $is_chair && $five_days_expired ? '<strong>Chair Can\'t Drop</strong>' : (!$signup_lock ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Take Me Off\">Take Me Off</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : '<strong>Signup Closed</strong>');
				...
				$driver = !$signup_hardlock ? "<div id=\"event_driver\"><form action=\"event.php\" method=\"post\">I can drive <input type=\"text\" name=\"driving\" size=\"2\" value=\"$row[driver]\" /> people including myself. <button type=\"submit\" name=\"function\" value=\"Edit Rides\">Edit Rides</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></div>" : '';
			} else if ($g_user->is_logged_in()) {
				$signup = !$signup_lock ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Signup\">Signup</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : '<strong>Signup Closed</strong>';
				$make_me_chair = !$signup_hardlock && ($chair_count < $max_chairs || $attendee_count == 0) ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Make Me Chair\">Make Me Chair</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : '';
				$take_me_off = '';
			...
			$evaluate = $is_chair && $now > $event_time || $g_user->permit("calendar add evaluations") && $now > $event_time ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Evaluate Event\">Evaluate Event</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : '';
...
<tr><th axis="name">Name</th><th axis="pledgeclass">Pledge Class</th><th axis="driving">Driving</th><th axis="phone">Phone</th><th axis="cell">Cell</th><th axis="signup">Signed Up</th>$remove_attendee_heading</tr>
$attendees
$waitlist
</table>
<hr />
...
<div><button type="submit" name="function" value="Post Comment">Post Comment</button></div>
</form>
</div>
</div>
...
<select name="timestamp">$select_month_options</select> <input type="submit" value="Go!" /> <a href="service_calendar_chapter.php?year=$prev_year&month=$prev_month">Prev</a> <a href="service_calendar_chapter.php?year=$next_year&month=$next_month">Next</a>
...
<select name=navi onkeypress="location.href = this.form.navi.options[this.form.navi.selectedIndex].value">
	<option value="calendar.php?year=$this_year&month=$this_month">Calendar
	<option value="service_calendar.php?year=$this_year&month=$this_month">Service Calendar
	<option value="#" selected="selected">Service Calendar - Chapter
...
<select name="timestamp">$select_month_options</select> <input type="submit" value="Go!" /> <a href="service_calendar.php?year=$prev_year&month=$prev_month">Prev</a> <a href="service_calendar.php?year=$next_year&month=$next_month">Next</a>
</form>
...
DOCHERE_print_upcoming_events;
	...
	function process_add_evaluation() {
		...
		while ($row = $query->fetch_row()) {
			...
			if (isset($_POST["hours$user_id"]) && $_POST["hours$user_id"] && isset($_POST["hoursAmount$user_id"]) && is_numeric($_POST["hoursAmount$user_id"])) {
				if ($_POST["hoursAmount$user_id"] <= 0) {
			...
			$query2 = new Query(sprintf("UPDATE %scalendar_attend SET attended=%s, chair=%s, flaked=%s, driver=%d, hours=%d WHERE event_id=%s AND user_id=%s LIMIT 1",
				TABLE_PREFIX, $attended, $chaired, $flaked, $driver, $hours, $event_id, $user_id));
			$query2 = new Query(sprintf("INSERT INTO %sevent_audit_trail SET event_id=%d, user_id=%d, target_user_id=%d, timestamp='%s', description='%s'",
					TABLE_PREFIX, $event_id, $g_user->data['user_id'], $user_id, $timestamp, $description));
		}
	...
	function process_add_event() {
		...
		if (!$error) {
			$insert_statement = sprintf("INSERT INTO %scalendar_event SET title='%s', location='%s', 
				...
				signup_begin=%s, signup_cutoff=%s, signup_limit=%d, signup_lock=FALSE",
			...
			$timestamp = date("Y-m-d H:i:s");
			$description = "Event created: " . Query::escape_string($insert_statement);
			$query = new Query("start transaction");
	...
	function query_range($begin, $end) {
		...
		return new Query(sprintf("SELECT %s FROM %scalendar_event%s WHERE %s AND (type_service_chapter = 1 or type_fundraiser = 1) ORDER BY date ASC", $select_expression, TABLE_PREFIX, $join_expression, $where_expression));
	}
}