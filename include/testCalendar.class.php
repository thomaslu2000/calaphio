...
DOCHERE_print_evaluation;
	...
	function print_event() {
		...
		if ($row = $query->fetch_row()) {
			...
			$event_time = strtotime($row['date']);
			$date = date("l, M d, Y", $event_time);
			$location = $row['location'] && $g_user->is_logged_in() ? " @ " . $row['location'] : "";
			$signup_limit = $row['signup_limit'] == 0 ? "No Max!" : $row['signup_limit'];
			$description = $this->format_add_links($row['description']);
			$signup_begin_time = $row['signup_begin'] ? strtotime($row['signup_begin']) : false;
			$signup_cutoff_time = $row['signup_cutoff'] ? strtotime($row['signup_cutoff']) : strtotime("-3 days", strtotime($row['date']));
			...
			if ($row = $query->fetch_row()) {
				...
				$photographer = !$is_photographer ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Make Me Photographer\">Make Me Photographer</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Remove As Photographer\">Remove As Photographer</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>";
				...
				$make_me_chair = !$signup_lock && ($chair_count < $max_chairs || $attendee_count == 0) ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Make Me Chair\">Make Me Chair</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : '';
				$take_me_off = '';
				if (!$signup_hardlock) {
					$photographer = !$is_photographer ? "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Make Me Photographer\">Make Me Photographer</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>" : "<li><form action=\"event.php\" method=\"post\"><button type=\"submit\" name=\"function\" value=\"Remove As Photographer\">Remove As Photographer</button><input type=\"hidden\" name=\"id\" value=\"$event_id\" /></form></li>";
				} else {
...
<select name="timestamp">$select_month_options</select> <input type="submit" value="Go!" /> <a href="calendar.php?year=$prev_year&month=$prev_month">Prev</a> <a href="calendar.php?year=$next_year&month=$next_month">Next</a>
			...
			for ($i = 0; $i < 7; $i++) {
				...
				while ($row && strtotime($row['date']) <= $current_day) {
					if (($row['type_service_chapter'] | $row['type_service_campus'] | $row['type_service_community'] | $row['type_service_country']) && ! $row['type_fundraiser']) {
						$service_items .= "        <li>" . $this->format_event_title($row) . "</li>\r\n";
					} elseif ($row['type_fellowship']) {
						$fellowship_items .= "        <li>" . $this->format_event_title($row) . "</li>\r\n";
					} elseif ($row['type_fundraiser']) {
						$fundraiser_items .= "        <li>" . $this->format_event_title($row) . "</li>\r\n";
					} else {
						$event_items .= "        <li>" . $this->format_event_title($row) . "</li>\r\n";
					}
					$row = $query->fetch_row();
				}
...
DOCHERE_print_upcoming_events;
	...
	function process_add_evaluation() {
		...
		if (!isset($_POST['event_hours']) || !is_numeric($_POST['event_hours'])) {
			...
			return false;	
		} else if ($_POST['event_hours'] <= 0 && $g_user->data['user_id'] != 1190 && $g_user->data['user_id'] != 1070 && $g_user->data['user_id'] != 1078) {
