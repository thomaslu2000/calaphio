...
if (!$g_user->is_logged_in() || !$g_user->permit("admin view requirements")) {
	...
	$query = new Query(sprintf("SELECT apo_users.user_id, firstname, lastname, pledgeclass FROM apo_users join apo_actives_sp12 using (user_id) order by lastname, firstname"));
	...
	while ($peopleRow = $query->fetch_row()) {
		...
		if ($service_total < 4) {
			$four_cs = "<FONT COLOR='RED'>4 Cs of Service: $service_total Cs <br/></FONT>";
		} else {
			$four_cs = "4 Cs of Service: $service_total Cs <br/>";
		}		
