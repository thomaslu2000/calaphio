...
select *, attendedTime + COALESCE(flakedTime, 0 )as totalTime from (SELECT firstname, lastname, user_id, pledgeclass, sum(hours) as attendedTime FROM (apo_calendar_event JOIN apo_calendar_attend USING (event_id)) Join (apo_users join apo_pledges using (user_id)) USING (user_id)  WHERE (type_service_chapter=TRUE OR type_service_campus=TRUE OR type_service_community=TRUE OR type_service_country=TRUE OR type_fundraiser=TRUE) AND attended = TRUE AND deleted=FALSE AND date BETWEEN '%s' AND '%s' Group By user_id) as attendedHours left join (SELECT user_id, sum(hours) * -1 as flakedTime FROM (apo_calendar_event JOIN apo_calendar_attend USING (event_id)) Join (apo_users join apo_pledges using (user_id)) USING (user_id)  WHERE (type_service_chapter=TRUE OR type_service_campus=TRUE OR type_service_community=TRUE OR type_service_country=TRUE OR type_fundraiser=TRUE) AND flaked = TRUE AND deleted=FALSE AND date BETWEEN '%s' AND '%s' Group By user_id) as flakedHours using (user_id) group by user_id order by totalTime DESC", date("Y-m-d", strtotime($start)), date("Y-m-d", strtotime($end)), date("Y-m-d", strtotime($start)), date("Y-m-d", strtotime($end))));
...
DOCHERE;
	...
	$query = new Query(sprintf("
	select *, attendedTime + COALESCE(flakedTime, 0 ) as totalTime from (SELECT firstname, lastname, user_id, pledgeclass, count(hours) as attendedTime FROM (apo_calendar_event JOIN apo_calendar_attend USING (event_id)) Join (apo_users join apo_pledges using (user_id)) USING (user_id)  WHERE (type_fellowship=TRUE) AND attended = TRUE AND deleted=FALSE AND date BETWEEN '%s' AND '%s' AND disabled = 0 Group By user_id) as attendedHours left join (SELECT user_id, count(hours) * -1 as flakedTime FROM (apo_calendar_event JOIN apo_calendar_attend USING (event_id)) Join (apo_users join apo_pledges using (user_id)) USING (user_id)  WHERE (type_fellowship = TRUE) AND flaked = TRUE AND deleted=FALSE AND date BETWEEN '%s' AND '%s' AND disabled = 0 Group By user_id) as flakedHours using (user_id) group by user_id order by totalTime DESC
...
DOCHERE;
	...
	$query = new Query(sprintf("SELECT count(chair) as num_chaired, firstname, lastname, pledgeclass FROM apo_calendar_event
			JOIN apo_calendar_attend USING (event_id)
			JOIN (apo_users JOIN apo_pledges using (user_id)) USING (user_id)
