...
if (!$g_user->is_logged_in()) {
	...
	$files = array();
	$dir = opendir("testbank/Environmental Design");
	while (false !== ($filename = readdir($dir))) {
		if (is_file("testbank/Environmental Design/$filename")) {
			$files[] = $filename;
		}
	}
	natcasesort($files);
	foreach ($files as $filename) {
		...
		echo "<li><a href=\"testbank/Environmental Design/$escaped_filename\">$escaped_filename</a></li>\n";
	}
	echo "</ul>\n";
	echo <<<HEREDOC
