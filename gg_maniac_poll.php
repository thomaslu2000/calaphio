...
$table_rows = '<tr>
			   ...
			   <tr>';
$query = new Query(sprintf("SELECT concat(apo_users.firstname, ' ', apo_users.lastname), gg_maniac_votes.name, gg_maniac_votes.reason FROM apo_users INNER JOIN gg_maniac_votes ON apo_users.user_id = gg_maniac_votes.user_id WHERE poll_id=%s", $_REQUEST['id']));
while ($row = $query->fetch_row()) {
	$fullname = $row['concat(firstname, ' ', lastname)'];
	$name = $row['name'];
	$reason = $row['reason'];
	$table_rows .= <<<DOCHERE
					<tr>
