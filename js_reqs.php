...
if ($g_user->data['user_id'] == 1142) {
	...
	$is_pm = false;
			...
			echo <<<DOCHERE
<h1>JS pledge class requirements for MH semester</h1>
<br/>
DOCHERE;
...
if (!$g_user->is_logged_in()) {
	...
	trigger_error("You must be the pledgemaster to access this feature", E_USER_ERROR);
} else {
	
	$query = new Query(sprintf("SELECT apo_users.user_id, firstname, lastname, pledgeclass FROM apo_users join apo_actives using (user_id) where pledgeclass='JS' and depledged=0 order by lastname, firstname"));
	...
	while ($peopleRow = $query->fetch_row()) {
		...
		if ($fellowship_events_count < 7) {
			$fellowship_events = "<FONT COLOR='RED'>7 Fellowships: $fellowship_events_count Fellowships <br/></FONT>";
		} else {
			$fellowship_events = "7 Fellowships: $fellowship_events_count Fellowships <br/>";
		}
...
DOCHERE;
			...
			echo <<<DOCHERE
Pay your \$50 active dues (\$55 at CM3, \$65 at CM5) <br/>
DOCHERE;
}
?>
