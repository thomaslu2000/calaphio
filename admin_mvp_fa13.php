...
if (!$g_user->is_logged_in() || !$g_user->permit("admin view requirements")) {
	...
	$query = new Query(sprintf("SELECT apo_users.user_id, firstname, lastname, pledgeclass FROM apo_users join apo_actives_fa13 using (user_id) order by lastname, firstname"));
