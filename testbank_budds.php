...
if (!$g_user->is_logged_in()) {
	...
	$files = array();
	$dir = opendir("testbank/Buddhist Studies");
	while (false !== ($filename = readdir($dir))) {
		if (is_file("testbank/Buddhist Studies/$filename")) {
			$files[] = $filename;
		}
	}
	natcasesort($files);
	foreach ($files as $filename) {
		...
		echo "<li><a href=\"testbank/Buddhist Studies/$escaped_filename\">$escaped_filename</a></li>\n";
	}
	echo "</ul>\n";
	echo <<<HEREDOC
